...
def create_rental_from_gui(client_name, account_id, total_minutes, info):
    """Создает запись об аренде из GUI."""
    try:
        ...
        logging.info(f"Аренда для '{client_name}' успешно создана из GUI.")
        return True
...
def move_rental_to_history(rental_id):
    """Перемещает аренду в историю и освобождает аккаунт."""
    try:
...
def remove_game(game_id):
    """Удаляет игру, если к ней не привязаны аккаунты."""
...
def import_accounts_from_csv(file_path):
    """Импортирует аккаунты из CSV файла."""
...
# --- Функции, используемые и ботом, и GUI ---
...
# ... (остальные функции из предыдущей версии db_handler.py остаются без изменений)
def rent_account(game_name, client_name, minutes, chat_id):
    """Находит свободный аккаунт, сдает в аренду и возвращает данные для входа."""
    ...
    remind_time = end_time - timedelta(minutes=10)  # 10-минутное напоминание
    ...
    logging.info(f"Аккаунт {login} ({game_name}) успешно сдан в аренду клиенту {client_name} на {minutes} минут.")
    return login, password, game_id


def check_and_process_expired_rentals():
    """Проверяет истекшие аренды, перемещает в историю и освобождает аккаунты."""
...
def get_rentals_for_reminder():
    """Находит все активные аренды, у которых прошло время напоминания, но напоминание еще не было отправлено."""
    now_iso = datetime.now().isoformat()
    return db_query(
        "SELECT id, client_name, funpay_chat_id FROM rentals WHERE remind_time <= ? AND is_history = 0 AND reminded = 0 AND pre_reminded = 0",
        (now_iso,),
        fetch="all"
    )
...
def mark_rental_as_reminded(rental_id):
    """Помечает аренду как 'напомненную', чтобы избежать повторных уведомлений."""
...
def extend_user_rental(username, hours_to_add):
    ...
    logging.info(
        f"Аренда для {username} продлена на {hours_to_add} ч. Новое время окончания: {new_end_time.isoformat()}")
    return new_end_time
